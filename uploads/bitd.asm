;add macro ;viva questions: related to addresses for output, tell why addresses change or stay constant for certain registers and output
;added:		;check for segemntation fault in  this progrtam 
%macro rw 4
	mov rax , %1
	mov rdi , %2
	mov rsi , %3
...
section .bss	
	...
	abc resq 1 				; quad word 64 bits , add similar vairbales for ldtr,tr,idtr,msw
	abclim resw 1
	temp64 resq 1
	temp16 resw 1
	asc resb 1
...
_start:
	...
	mov rsi, abc ;use pqr,xyz or for ldtr,idtr,tr, msw,...
	sgdt [rsi] ;Store GDTR contents into RSI destination: 8 bytes, operand: 8+2 bytes ;use sldt for ldtr, sidt for idtr , str for tr , smsw for masw
